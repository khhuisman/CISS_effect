...
# Onsite SOC Hamiltonian is generated, only d orbitals feel SOC.
...
# 
def make_fcc_lead_magnetized_voltage(nlayers,V, Txz,Tyz,d_L= 12, xi_d=0.6,delta_e=0,B=1,quantization_axis=sigma_z,rho_L=0.5,rho_R=0.5):
    ...
    """ 
     Input:
    - nlayers = thickness of the layers of gold atoms that feel SOC
    - V = applies bias voltage. Shift the onsite energies of the gold lead. Positive for the left lead and negatively for the right lead.
    - Txz, Tyz = dimensions of the plain gold atoms that feel SOC
    - d_L = distance between the two blocks of gold atoms.
    - xi_d = SOC parameter of 5d orbitals of gold
    - delta_e = shift in onsite energies of gold to align it to the proper fermi level.
    ...
    # Make a block of shape nlayers*Txz*Tyz with onsite soc xi_d.
    def sys_gold_volt(nlayers,V,Txz,Tyz,xi_d,Txy,rho=0.5):

        sys = kwant.Builder()  
        
        #Onsite Hamiltonian depends on voltage
        H_onsite = np.add(H_at_ev_soc,H_SO(xi_d)) + delta_e*np.identity(18) + rho*(V)*np.identity(18)
#         
    ...
    # Left block:  +V/2 at Txy =0
    sys = sys_gold_volt(nlayers,V,Txz,Tyz,xi_d,0,rho_L)
    
    # Right block: -V/2 at Txy =d_L
    sys_reversed = sys_gold_volt(nlayers,-V,Txz,Tyz,xi_d,d_L,rho_R)

    sys.update(sys_reversed)
    
    
    
    # Left Lead: has label 0 when calculating transmissions
    lead1_sym =  kwant.TranslationalSymmetry((-0.5,-0.5,0))
    lead1 = kwant.Builder(lead1_sym, conservation_law = sigmaz_18)
    lead1[(lat_fcc(0,j,k)  
                         for j in range(0,Txz) 
                         for k in range(0,Tyz))] = H_at_ev_soc + delta_e*np.identity(18) + (V/2)*np.identity(18)
    ...
    # Right lead magnetized: has label 1 when calculating transmissions
    lead2_sym =  kwant.TranslationalSymmetry((0.5,0.5,0))
    lead2 = kwant.Builder(lead2_sym, conservation_law = sigmaz_18)
    lead2[(lat_fcc(d_L+nlayers,j,k)  
                         for j in range(0,Txz) 
                         for k in range(0,Tyz))] = H_at_ev_soc+ delta_e*np.identity(18) + B*sigmazz_18 - (V/2)*np.identity(18)
...
def make_fcc_lead_chiral_hop_magnetized_voltage(nlayers,V, Txz,Tyz,d_L= 12, xi_d=0.6,delta_e=0,B=1,quantization_axis=sigma_z):
    ...
    # Make a block of shape nlayers*Txz*Tyz with onsite soc xi_d.
    def sys_gold_volt(nlayers,V,Txz,Tyz,xi_d,Txy):

        sys = kwant.Builder()  
        
        #Onsite Hamiltonian depends on voltage
        H_onsite = np.add(H_at_ev_soc,H_SO(xi_d)) + delta_e*np.identity(18) + (V/2)*np.identity(18)
#         
    ...
    # +V/2 at Txy =0
    sys = sys_gold_volt(nlayers,V,Txz,Tyz,xi_d,0)
    
    # -V/2 at Txy =d_L
    sys_reversed = sys_gold_volt(nlayers,-V,Txz,Tyz,xi_d,d_L)
